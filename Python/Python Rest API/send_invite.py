...
async def send_invitation():
    ...
    for i, row in df.iterrows():
        ...
        print(f"searching for guest {new_email} in azure portal...")

        query_params = UsersRequestBuilder.UsersRequestBuilderGetQueryParameters(
        filter = f"mail eq '{new_email}' or otherMails/any(o:o eq '{new_email}')",
        )
        ...
        try:
            ...
            logger.error(f"Error occurred while searching for {new_email}: {e}")
            continue
        ...
        if result.value:
            ...
            if DRY_RUN:
                logger.info(f"DRY RUN invite to {new_email}")
            else:
                # send redemption invitation to new email
                try:
